...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    httpOnly: true, // cookie cannot be modifoed by browser
  };
...
exports.isLoggedIn = async (req, res, next) => {
  let token;
  console.log('reqcookiesjwt:: ', req.cookies.jwt);
  if (req.cookies.jwt) {
    try {
      // verify token
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // const message = `Forgot your Password? Submit a PATCH request with confirm password and new password to: ${resetURL}.\nIf you didn't forgot you password please ignore this email!`;
  ...
  try {
    ...
    return next(
      new AppError('Their was error sending email. Try again later!'),
      500
    );
  }
  // next();
